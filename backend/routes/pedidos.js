...
router.post("/", (req, res) => {
  const { usuarioId, cliente, sedeId, tipoEntrega, direccion, productos } = req.body;

  if (!sedeId || !productos || !Array.isArray(productos) || productos.length === 0) {
    return res.status(400).json({ mensaje: "El pedido debe tener sede y productos" });
  }

  if (!cliente || !cliente.nombre || !cliente.telefono) {
    return res.status(400).json({ mensaje: "Completa el nombre y teléfono del cliente" });
  }
  ...
  if (!["recojo", "delivery"].includes(tipoEntrega || "recojo")) {
    return res.status(400).json({ mensaje: "Tipo de entrega inválido" });
  }

  if (tipoEntrega === "delivery" && !String(direccion || "").trim()) {
    return res.status(400).json({ mensaje: "Ingresa una dirección para delivery" });
  }

  const sedes = leerJSON(sedesPath);
  const sedeExiste = sedes.some((sede) => Number(sede.id) === Number(sedeId));
  ...
  try {
    productosValidados = productos.map((item) => {
      const producto = catalogo.find((p) => Number(p.id) === Number(item.productoId));
      ...
      const precioUnitario = Number(producto.precios[item.tamaño]);
  ...
  const total = productosValidados.reduce((sum, item) => sum + item.subtotal, 0);
  const pedidos = leerJSON(filePath);

  const nuevoPedido = {
    id: pedidos.length > 0 ? Math.max(...pedidos.map((p) => Number(p.id) || 0)) + 1 : 1,
    ...
    direccion: tipoEntrega === "delivery" ? String(direccion || "").trim() : "",
    productos: productosValidados,
    subtotal: total,
    delivery: tipoEntrega === "delivery" ? 6 : 0,
    total: total + (tipoEntrega === "delivery" ? 6 : 0),
    estado: "pendiente",
...
router.patch("/:id/estado", requerirAdmin, (req, res) => {
  ...
  const pedidos = leerJSON(filePath);
  const pedido = pedidos.find((item) => Number(item.id) === Number(req.params.id));
...
router.delete("/:id", requerirAdmin, (req, res) => {
  const pedidos = leerJSON(filePath);
  const nuevosPedidos = pedidos.filter((item) => Number(item.id) !== Number(req.params.id));
